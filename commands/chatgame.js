...
if (item.type === 'boolean') game.setDescription(`TRUE OR FALSE: ${item.question}`).setFooter(`You have the next 5 minutes to answer this question! | Difficulty: ${item.difficulty}`).setColor('RANDOM');
    ...
    bot.channels.cache.get('702459181973635123').send(game).then((lastEmbed) => {
 let collector = lastEmbed.channel.createMessageCollector(filter, { max: 1, time: 280000, errors: ['time'] })
		collector.on('collect', collected => {
     bot.channels.cache.get('702459181973635123').send(`${collected.author} got the correct answer!`)
  })
		collector.on('end', collected => {
      if (collected.size === 0) {
			return bot.channels.cache.get('702459181973635123').send('Looks like nobody got the answer this time. The correct answer was: **' + item.correct_answer + '**');
      }
      else {
